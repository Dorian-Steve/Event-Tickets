...
export const getQueuePosition = query({
    ...
    handler: async (ctx, { eventId, userId }) => {
      ...
      const peopleAhead = await ctx.db
        ...
        .filter((q) =>
          q.and(
            // get all entires before this one
 ...
 */
  ...
  export const releaseTicket = mutation({
    ...
    handler: async (convexToJson, {eventId, waitingListId }) => {
      const entry = await convexToJson.db.get(waitingListId);

      if (!entry || entry.status !==WAITING_LIST_STATUS.OFFERED) {
        throw new Error("No valid ticket offer found");
      }

      await convexToJson.db.patch(waitingListId, {
        status: WAITING_LIST_STATUS.EXPIRED,
      });

      // Process queue to offer ticket to next person
      await processQueue(convexToJson, { eventId});
    },
  });