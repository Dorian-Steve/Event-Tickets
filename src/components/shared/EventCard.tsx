...
function EventCard({ eventId }: {eventId: Id<"events"> }){
  ...
  const event = useQuery(api.event.getById, { eventId});

  const availability = useQuery(api.event.getEventAvailability, { eventId});
    ...
    if (userTicket) {
      return (
        <div className="mt-4 flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100">
          ...
          className='text-sm bg-green-600 hover:bg-green-700 text-white px-4 py-1.5 rounded-full font-medium shadow-sm transition-colors duration-200 flex items-center gap-1'>
            View your ticket
          </button>
        </div>
      );
  ...
  return (
  <div onClick={() => router.push(`/events/${eventId}`)}
      ...
      <div className={`p-6 ${imageUrl ? "relative" : ""}`}>
        <div className="flex justify-between items-start">
          ...
          <div>
            <div className="flex flex-col items-start gap-2">
              {isEventOwner && (
                <span className="inline-flex items-center gap-1 bg-blue-600/90 
        ...
        <div className="flex flex-col items-end gap-2 ml-4">
          <span className={`px-4 py-1.5 font-semibold rounded-full ${
            isPastEvent ? "bg-gray-50 text-gray-500"
            : "bg-green-50 text-green-700"
          }`}>
            {event.price.toFixed(2)} CFA
          </span>
